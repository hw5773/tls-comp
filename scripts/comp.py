...
def usage():
    ...
    print ("python3 comp.py <input directory> <output file>")
    exit()
...
def main():
    ...
    of.write("domain, size, zlib, zlib ratio, brotli, brotli ratio\n")
    max_size = -1
    min_size = 1000000
    num = 0
    ...
    for root, dirs, files in os.walk(d):
        for f in files:
            ...
            s = "%s, %d, %d, %f, %d, %f\n" % (dom, size, len(a), round(len(a)/size, 2), len(b), round(len(b)/size, 2))
            print (num, ") ", s)
            of.write(s)
